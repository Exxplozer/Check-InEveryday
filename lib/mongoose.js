...
exports.FindCurrentCheckin = function(){
    var date = new Date();
    Checkin.find({checkinDate: { $gte: new Date().setHours(newdate.getHours() + parseInt(data[i].interval))}, count: { $gte: 0}}, function (err, data) {
        for(var i =0; i < data.length;i++){
            ...
            var newdate = new Date(data[i].checkinDate);
            newdate.setHours(newdate.getHours() + parseInt(data[i].interval));

            data[i].checkinDate = newdate;
